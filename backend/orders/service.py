...
async def create_order_with_calculation(
    ...
    try:
        ...
        calc_result = await call_calculator_service(
            ...
            is_need_special_equipment=order_data.is_need_special_equipment,
            # add for electroplating_auto service
            electroplating_family = order_data.electroplating_family,
            electroplating_process_id = order_data.electroplating_process_id,
            coating_thickness_microns = order_data.coating_thickness_microns,
            processing_depth_microns = order_data.processing_depth_microns,
...
async def recalculate_order_price(db: AsyncSession, order: models.Order) -> bool:
    ...
    try:
        ...
        calc_result = await call_calculator_service(
            ...
            # add for electroplating_auto service
            electroplating_family = order.electroplating_family,
            electroplating_process_id = order.electroplating_process_id,
            coating_thickness_microns = order.coating_thickness_microns,
            processing_depth_microns = order.processing_depth_microns,
            k_otk=order.k_otk,
            k_cert=order.k_cert,
            timeout=10.0,
            file_data=file_data,

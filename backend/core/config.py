...
ADMIN_PERSONAL_EMAIL = os.getenv("ADMIN_PERSONAL_EMAIL", "admin@system.local")
ADMIN_DEFAULT_PASSWORD = os.getenv("ADMIN_DEFAULT_PASSWORD", "admin")
ADMIN_EMAIL = os.getenv("ADMIN_EMAIL", ADMIN_PERSONAL_EMAIL)
...
EMAIL_VERIFICATION_CONFIRM_URL_TEMPLATE = os.getenv(
    "EMAIL_VERIFICATION_CONFIRM_URL_TEMPLATE",
    "https://app.example.com/confirm-email?token={token}",
)
EMAIL_VERIFICATION_SEND_COOLDOWN_SECONDS = int(os.getenv("EMAIL_VERIFICATION_SEND_COOLDOWN_SECONDS", "300"))
EMAIL_VERIFICATION_RATE_LIMIT_PER_EMAIL = int(os.getenv("EMAIL_VERIFICATION_RATE_LIMIT_PER_EMAIL", "3"))
EMAIL_VERIFICATION_RATE_LIMIT_PER_IP = int(os.getenv("EMAIL_VERIFICATION_RATE_LIMIT_PER_IP", "20"))
EMAIL_VERIFICATION_RATE_LIMIT_WINDOW_SECONDS = int(os.getenv("EMAIL_VERIFICATION_RATE_LIMIT_WINDOW_SECONDS", "3600"))
EMAIL_VERIFICATION_BITRIX_SUBJECT = os.getenv(
    "EMAIL_VERIFICATION_BITRIX_SUBJECT",
    "Подтверждение электронной почты",
)
EMAIL_VERIFICATION_BITRIX_MESSAGE_FROM = os.getenv(
    "EMAIL_VERIFICATION_BITRIX_MESSAGE_FROM",
    "service.bitrix24 <service.bitrix24@kronshtadt.ru>",
)
EMAIL_VERIFICATION_BITRIX_RESPONSIBLE_ID = int(os.getenv("EMAIL_VERIFICATION_BITRIX_RESPONSIBLE_ID", "1"))
EMAIL_VERIFICATION_REQUIRE_BITRIX_CONTACT = (
    os.getenv("EMAIL_VERIFICATION_REQUIRE_BITRIX_CONTACT", "true").lower() == "true"
)

# Password recovery (forgot password)
PASSWORD_RECOVERY_ENABLED = os.getenv("PASSWORD_RECOVERY_ENABLED", "true").lower() == "true"
PASSWORD_RECOVERY_TOKEN_TTL_SECONDS = int(os.getenv("PASSWORD_RECOVERY_TOKEN_TTL_SECONDS", "3600"))
PASSWORD_RECOVERY_RESET_URL_TEMPLATE = os.getenv(
    "PASSWORD_RECOVERY_RESET_URL_TEMPLATE",
    "https://app.example.com/reset-password?token={token}",
)
PASSWORD_RECOVERY_SEND_COOLDOWN_SECONDS = int(os.getenv("PASSWORD_RECOVERY_SEND_COOLDOWN_SECONDS", "300"))
PASSWORD_RECOVERY_RATE_LIMIT_PER_EMAIL = int(os.getenv("PASSWORD_RECOVERY_RATE_LIMIT_PER_EMAIL", "3"))
PASSWORD_RECOVERY_RATE_LIMIT_PER_IP = int(os.getenv("PASSWORD_RECOVERY_RATE_LIMIT_PER_IP", "20"))
PASSWORD_RECOVERY_RATE_LIMIT_WINDOW_SECONDS = int(os.getenv("PASSWORD_RECOVERY_RATE_LIMIT_WINDOW_SECONDS", "3600"))
PASSWORD_RECOVERY_BITRIX_SUBJECT = os.getenv(
    "PASSWORD_RECOVERY_BITRIX_SUBJECT",
    "Восстановление пароля",
)
PASSWORD_RECOVERY_BITRIX_MESSAGE_FROM = os.getenv(
    "PASSWORD_RECOVERY_BITRIX_MESSAGE_FROM",
    "service.bitrix24 <service.bitrix24@kronshtadt.ru>",
)
PASSWORD_RECOVERY_BITRIX_RESPONSIBLE_ID = int(os.getenv("PASSWORD_RECOVERY_BITRIX_RESPONSIBLE_ID", "1"))
PASSWORD_RECOVERY_REQUIRE_BITRIX_CONTACT = (
    os.getenv("PASSWORD_RECOVERY_REQUIRE_BITRIX_CONTACT", "true").lower() == "true"
)
